...
export async function GET(request: Request) {
  ...
  // 1. Fetch all scraped jobs from the last 30 days
  const thirtyDaysAgo = new Date();
  thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

  const { data: scrapedJobs, error: jobsError } = await supabase
    .from("scraped_jobs")
    .select("id, company, title, location, url, description, posted_at, keywords, contact_id, contact_alias")
  ...
  for (const job of scrapedJobs || []) {
    ...
    // Avoid duplicate URLs or IDs in the list
